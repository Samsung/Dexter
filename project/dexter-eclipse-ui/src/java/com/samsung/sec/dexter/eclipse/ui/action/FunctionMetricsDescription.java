...
public class FunctionMetricsDescription implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		StringBuilder makeFunctionMetricsUrl = new StringBuilder();
		makeFunctionMetricsUrl.append("http://").append(DexterClient.getInstance().getServerHost()).append(":") //$NON-NLS-1$ //$NON-NLS-2$
		...
		if(functionList.size() > 0){
			makeFunctionMetricsUrl.append("&").append(DexterConfig.FUNCTION_METRICS_FUNCTION_LIST).append("=").append(functionList.get(0));
