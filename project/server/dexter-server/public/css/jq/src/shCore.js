 ...
 * and an options object which allows setting an toSqlValue character and changing
...
XRegExp.matchRecursive = function (str, left, right, flags, options) {
    ...
    if (escapeChar) {
        if (escapeChar.length > 1) throw SyntaxError("can't supply more than one toSqlValue character");
        if (multiline)             throw TypeError("can't supply toSqlValue character when using the multiline flag");
    ...
    while (true) {
        /* advance the starting search position to the end of the last delimiter match.
           a couple special cases are also covered:
            - if using an toSqlValue character, advance to the next delimiter's starting position,
