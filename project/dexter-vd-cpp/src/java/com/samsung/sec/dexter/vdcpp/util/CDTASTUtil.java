...
public class CDTASTUtil {
	...
	public static void fillDefectData(final AnalysisConfig config,
		...
		PreOccurence preOcc = createPreOccurence(config, checker, translationUnit, fileLocation, message,declaratorName);
	...
	public static boolean isIntegerType(IType type){
		...
		IBasicType.Kind kind=((IBasicType) type).getKind();
		return (kind==IBasicType.Kind.eInt && !((IBasicType) type).isShort() && !((IBasicType) type).isLong() && !((IBasicType) type).isLongLong());
	}
	...
	private static String getValueOfIdExpression(IASTExpression expression) {
		String value="";
