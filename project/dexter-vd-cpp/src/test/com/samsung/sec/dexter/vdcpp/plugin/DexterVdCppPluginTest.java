	...
	public void analyze_method_for_SIGNED_UNSIGNED_ASSIGNMENT_ERROR(){		
		...
		for(Defect defect : result.getDefectList()){
			if(defect.getCheckerCode().equals("SIGNED_UNSIGNED_ASSIGNMENT_ERROR"))
			{
				...
				assertEquals("[#1@28] A Signed data type is transformed into a larger Unsigned data type. This can produce unexpected values. [#2@29] A Signed data type is transformed into a larger Unsigned data type. " +
						"This can produce unexpected values. [#3@30] A Signed data type is transformed into a larger Unsigned data type. This can produce unexpected values. ", defect.getMessage());
	...
	public void analyze_method_for_CHECK_DUID(){		
		...
		for(Defect defect : result.getDefectList()){
			if(defect.getCheckerCode().equals("CHECK_USAGE_DUID"))
			{
				...
				Occurence occ = defect.getOccurences().get(0);				
				assertEquals(occ.getVariableName()+" is used. You need to check usage and the purpose of ID system. please refer to Samsung Smart TV Service Device Identifier Guideline", occ.getStringValue());				
	...
	public void analyze_method_for_AtoiReturnValueAsArrayIndex(){		
		...
		for(Defect defect : result.getDefectList()){
			if(defect.getCheckerCode().equals("CHECK_USAGE_ATOI_AS_ARRAY_INDEX"))
			{
				...
				Occurence occ = defect.getOccurences().get(0);	
				assertEquals("[#1@25] atoi() couldreturned return negative value(return type is integer value). do Not use returned value of atoi() " +
						"as array index without checking its range [#2@32] atoi() couldreturned return negative value(return type is integer value). " +
						"do Not use returned value of atoi() as array index without checking its range ", defect.getMessage());
	...
	public void analyze_method_for_NoFreeForThirdParameter(){		
		...
		for(Defect defect : result.getDefectList()){
			if(defect.getCheckerCode().equals("CHECK_FREE_STMT_THIRD_PARAM"))
			{
				assertEquals(3, defect.getOccurences().size());
				Occurence occ = defect.getOccurences().get(0);	
				assertEquals("[#1@20] You should have free statement for a parameter by calling app_control_get_extra_data " +
						"[#2@25] You should have free statement for a parameter by calling app_control_get_extra_data_array " +
						"[#3@31] You should have free statement for a parameter by calling app_control_get_extra_data_array ", defect.getMessage());
	...
	{		
		...
		for(Defect defect : result.getDefectList()){
			if(defect.getCheckerCode().equals("CHECK_ARRAY_INDEX_OUT_OF_BOUNDS_TWO_DIMESIONAL"))
			{
				assertEquals(2, defect.getOccurences().size());
				Occurence occ = defect.getOccurences().get(0);	
				assertEquals("[#1@26] Array index out of bounds [#2@34] Array index out of bounds " , defect.getMessage());
	...
	public void analyze_method_for_ForWhileControlBracket(){		
		...
		for(Defect defect : result.getDefectList()){
			if(defect.getCheckerCode().equals("FOR_WHILE_CONTROL_BRACKET"))
			{
				assertEquals(4, defect.getOccurences().size());
				Occurence occ = defect.getOccurences().get(0);	
				assertEquals("[#1@22] Although conditional or control statement (if/for/while) is single line, '{' and '}' shall be existed " +
						...
						"[#4@35] Although conditional or control statement (if/for/while) is single line, '{' and '}' shall be existed ", defect.getMessage());
	...
	public void analyze_method_for_AddFallThroughCommentOnSwitchCase(){		
		...
		for(Defect defect : result.getDefectList()){
			if(defect.getCheckerCode().equals("ADD_FALLTHROUGH_IN_SWITCHCASE"))
			{
				assertEquals(2, defect.getOccurences().size());
				Occurence occ = defect.getOccurences().get(0);	
				assertEquals("[#1@25] In switch statement, every case, except default, shall contain break/return statement. Omitting break/return intentionally, /* FALL THROUGH */ comment shall be used " +
	...
	public void analyze_method_for_IfElseControl(){		
		...
		for(Defect defect : result.getDefectList()){
			if(defect.getCheckerCode().equals("CHECK_IFELSE_CONTROL"))
			{
				assertEquals(3, defect.getOccurences().size());
				Occurence occ = defect.getOccurences().get(0);	
				assertEquals("[#1@27] 'else' and 'else if' in if-else or if-else if-else statement shall be started in the new line, " +
	...
	public void analyze_method_for_ForWhileNullBody(){		
		...
		for(Defect defect : result.getDefectList()){
			if(defect.getCheckerCode().equals("FOR_WHILE_NULL_BODY"))
			{
				...
				Occurence occ = defect.getOccurences().get(0);	
				assertEquals("[#1@21] When body is omitted intentionally in 'for/while' statement, it shall contain null statement(';') and comment, /* NULL */ " +
	...
	public void analyze_method_for_FunctionReturnValue(){		
		...
		for(Defect defect : result.getDefectList()){
			if(defect.getCheckerCode().equals("FUNCTION_RETURN_VALUE"))
			{
				assertEquals(1, defect.getOccurences().size());
				Occurence occ = defect.getOccurences().get(0);	
				assertEquals("[#1@20] Return type shall be explicitly used. Even if there is no return value, 'void' shall be used ",
						defect.getMessage());								
				assertEquals(20, occ.getStartLine());
				assertEquals(20, occ.getEndLine());	
	...
	public void analyze_method_for_MacroParenthesis(){		
		...
		for(Defect defect : result.getDefectList()){
			if(defect.getCheckerCode().equals("MACRO_PARENTHESIS"))
			{
				...
				Occurence occ = defect.getOccurences().get(0);	
				assertEquals("[#1@17] In macro definition, every expression and the arguments in the expressions shall be enclosed by '(' and ')' for each " +
